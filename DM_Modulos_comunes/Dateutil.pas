...
{$IFDEF RX_D3}
  lFormat:=TFormatSettings.Create; // Load system defaults.
  if lFormat.ShortDateFormat[1] = 'g' then { skip over prefix text }
    ScanToNumber(S, Pos);
{$ENDIF RX_D3}
  if not (ScanNumber(S, MaxInt, Pos, N1) and ScanChar(S, Pos, lFormat.DateSeparator) and
    ScanNumber(S, MaxInt, Pos, N2)) then Exit;
  if ScanChar(S, Pos, lFormat.DateSeparator) then begin
  ...
  ScanChar(S, Pos, lFormat.DateSeparator);
  ScanBlanks(S, Pos);
{$IFDEF RX_D3}
  if SysLocale.FarEast and (System.Pos('ddd', lFormat.ShortDateFormat) <> 0) then
  begin { ignore trailing text }
    if lFormat.ShortTimeFormat[1] in ['0'..'9'] then  { stop at time digit }
      ScanToNumber(S, Pos)
      ...
      until (Pos > Length(S)) or
        (AnsiCompareText(lFormat.TimeAMString, Copy(S, Pos, Length(lFormat.TimeAMString))) = 0) or
        (AnsiCompareText(lFormat.TimePMString, Copy(S, Pos, Length(lFormat.TimePMString))) = 0);
  end;
{$ENDIF RX_D3}
...
function MonthFromName(const S: string; MaxLen: Byte): Byte;
var lFormat: TFormatSettings;
begin
  lFormat:=TFormatSettings.Create; // Load system defaults.
  if Length(S) > 0 then
    for Result := 1 to 12 do begin
      if (Length(lFormat.LongMonthNames[Result]) > 0) and
        (AnsiCompareText(Copy(S, 1, MaxLen),
        Copy(lFormat.LongMonthNames[Result], 1, MaxLen)) = 0) then Exit;
    end;
  Result := 0;
end;
{
...
function StrToDateDef(const S: string; Default: TDateTime): TDateTime;
begin
  if not InternalStrToDate(TFormatSettings.ShortDateFormat, S, Result) then
...
begin
  ...
  if FourDigitYear then begin
    case GetDateOrder(lFormat.ShortDateFormat) of
...
initialization
  lFormat:=TFormatSettings.Create; // Load system defaults.
  FourDigitYear := Pos('YYYY', AnsiUpperCase(lFormat.ShortDateFormat)) > 0;
{$ENDIF}
end.